...
(define-private (send-nft (token-id uint) (recipient principal) (nft <sip009-nft-trait>))
	(as-contract (contract-call? nft transfer token-id tx-sender recipient))
)

(define-public (place-in-escrow (token-id uint) (recipient principal) (amount uint) (nft <sip009-nft-trait>))
	(begin
...
(define-public (pay-and-redeem (token-id uint) (nft <sip009-nft-trait>))
...
(define-public (cancel-escrow (token-id uint) (recipient principal) (nft <sip009-nft-trait>))
